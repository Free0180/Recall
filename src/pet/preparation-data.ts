import { validDaily, type DailyPlans } from "./daily-plan";
...
export interface PrepState { examMonth: string; examDate: string; weekdayMinutes: number; weekendMinutes: number; ket: string; priority: string; resources: string; completed: Record<string, boolean>; results: Result[]; phraseChecks: Record<string, boolean>; records: ManualRecord[]; lessons?: Record<string, LessonWork>; daily?: DailyPlans }
...
export function validPrep(value: unknown): value is PrepState {
  ...
  return typeof s.examMonth === "string" && /^20\d{2}-(0[1-9]|1[0-2])$/.test(s.examMonth) && typeof s.examDate === "string" && (s.examDate === "" || /^20\d{2}-\d{2}-\d{2}$/.test(s.examDate)) && [s.weekdayMinutes, s.weekendMinutes].every(n => Number.isInteger(n) && n >= 15 && n <= 240)
    ...
    && new Set(s.results.map(r => r.id)).size === s.results.length && new Set(s.records.map(r => r.id)).size === s.records.length && validLessons(s.lessons) && validDaily(s.daily);
}
