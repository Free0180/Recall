...
export function getBritishVoice(): SpeechSynthesisVoice | undefined {
  ...
  return english.find(voice => british(voice) && voice.localService)
    ?? english.find(voice => voice.localService)
    ?? english.find(british) ?? english[0];
}
...
export function speak(text: string, rate = 0.82, options: SpeechOptions = {}): void {
  ...
  if (options.preferAudio || /HuaweiBrowser|HUAWEI|HarmonyOS/i.test(navigator.userAgent) || !window.speechSynthesis) {
